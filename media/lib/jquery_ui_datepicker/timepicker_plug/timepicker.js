    ...
    return this.each( function() {
        ...
        function renderPickerPlug(b24Hour_,lang_) {
            ...
            if (!$('#pickerplug').length) {
                ...
                htmlins += '</li>';
                htmlins += '<li>';
                htmlins += '<div id="timepicker" class="ui-widget-content">';
                htmlins += '<h3 id="tpSelectedTime" class="ui-widget-header">';
                ...
                htmlins += '</li>';
                htmlins += '    <li>';
                htmlins += '<span class="ui-datepicker-close ui-icon ui-icon-closethick">close</span>';
                htmlins += '    </li>'
                htmlins += '</ul>';
                $('body').append(htmlins);

                $('#datepicker').datepicker();
                $("input#id_datetime").blur(closePickPlug);
                $(document).mousedown(closePickPlug);
                $(window).resize(closePickPlug);
        ...
        function closePickPlug (event) {
            if (event.type != "blur" && ($(event.target).parents('#pickerplug').length ||
