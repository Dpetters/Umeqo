...
class Client(local):
    ...
    def _init_buckets(self):
        ...
        for server in self.servers:
            for i in range(server.weight): #@UnusedVariable
    ...
    def delete_multi(self, keys, time=0, key_prefix=''):
        ...
        server_keys, prefixed_to_orig_key = self._map_and_prefix_keys(keys, key_prefix) #@UnusedVariable
    ...
    def _expect_cas_value(self, server, line=None):
        ...
        if line and line[:5] == 'VALUE':
            resp, rkey, flags, len, cas_id = line.split() #@UnusedVariable
    ...
    def _expectvalue(self, server, line=None):
        ...
        if line and line[:5] == 'VALUE':
            resp, rkey, flags, len = line.split() #@UnusedVariable
            flags = int(flags)
            rlen = int(len)
...
class _Host(object):
    ...
    def __init__(self, host, debug=0):
        ...
        if hostData.get('proto') == 'unix':
            self.family = socket.AF_UNIX #@UndefinedVariable
...
def _doctest():
    import doctest, memcache #@UnresolvedImport
